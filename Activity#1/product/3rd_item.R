...
url <- "https://www.amazon.com/s?k=jagger+pants+men&crid=1OENMOOZESER5&sprefix=jagger+p%2Caps%2C2985&ref=nb_sb_ss_ts-doa-p_2_8"
...
write.csv(product_df1, "3rd_item.csv")

product_df1$Category <- "pants"

product_df1 <- product_df1[, c("Category", names(product_df1)[-which(names(product_df1) == "Category")])]

write.csv(product_df1, "3rd_product.csv", row.names = FALSE)
