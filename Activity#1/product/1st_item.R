...
url <- "https://www.amazon.com/s?k=travis+scott+shoes&ref=nav_bb_sb"
...
write.csv(product_df1, "1st_item.csv")

product_df1$Category <- "shoes"

product_df1 <- product_df1[, c("Category", names(product_df1)[-which(names(product_df1) == "Category")])]

write.csv(product_df1, "1st_product.csv", row.names = FALSE)
